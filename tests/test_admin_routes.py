...
class TestAdvancedFeatures:
    ...
    def test_bulk_delete_action(self, auth_client, database):
        ...
        # --- ИЗМЕНЕНИЕ: Добавлены обязательные поля при создании деталей ---
...
class TestHierarchyFeatures:
    ...
    def test_add_child_part_successfully(self, auth_client, database):
        ...
        # --- ИЗМЕНЕНИЕ: Добавлено обязательное поле material ---
        form_data = {
            'part_id': 'CHILD-001',
            'product_designation': 'Дочерний узел', # Это поле в форме теперь отвечает за `name`
            'material': 'Алюминий',
            'quantity_total': 2,
            'csrf_token': 'fake-token'
        }
