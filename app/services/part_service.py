...
def create_child_part(form, parent_part_id, user):
    ...
    # TODO: Форма AddChildPartForm должна быть расширена полями name и material.
    new_part = Part(
        part_id=form.part_id.data,
        product_designation=parent_part.product_designation, # Наследует изделие родителя
        name=form.product_designation.data, # Поле product_designation в форме теперь - это name
        material=form.material.data if hasattr(form, 'material') else 'Не указан', # Новое поле
        quantity_total=form.quantity_total.data,
