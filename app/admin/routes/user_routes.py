...
def login():
    # <<< ИСПРАВЛЕНИЕ ЛОГИКИ ПОВТОРНОГО ЛОГИНА >>>
...
def edit_role(role_id):
    ...
    # Заполняем чекбоксы текущими правами роли
...
def add_user():
    form = AddUserForm()
    if form.validate_on_submit():
        # Улучшение: Проверяем, существует ли пользователь, чтобы избежать IntegrityError
...
def edit_user(user_id):
    ...
    if form.validate_on_submit():
        # Проверяем, не пытается ли пользователь переименовать себя в уже существующего
    ...
    # Предзаполняем поле роли текущей ролью пользователя
...
def delete_user(user_id):
    ...
    # Дополнительная проверка: не позволяем удалять единственного администратора
    admin_role = Role.query.filter_by(name='Administrator').first()
    if user_to_delete.role == admin_role and User.query.filter_by(role=admin_role).count() == 1:
