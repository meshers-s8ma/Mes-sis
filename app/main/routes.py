...
def api_parts_for_product(product_designation):
    ...
    for part in parts_query:
        # Расчет статусов этапов маршрута
        route_stages_data = []
        if part.route_template:
            completed_counts = Counter(h.status for h in part.history)
            ordered_stages = sorted(part.route_template.stages, key=lambda s: s.order)
            for rs in ordered_stages:
                stage_name = rs.stage.name
                status = 'pending'
                if completed_counts[stage_name] > 0:
                    status = 'completed'
                    completed_counts[stage_name] -= 1
                route_stages_data.append({'name': stage_name, 'status': status})
        
        parts_list.append({
            'part_id': part.part_id,
            'name': part.name,  # Добавлено поле
            'material': part.material,  # Добавлено поле
...
def select_stage(part_id):
    ...
    next_stage_obj = None
    for rs in ordered_stages:
        if completed_counts[rs.stage.name] > 0:
            completed_counts[rs.stage.name] -= 1
        else:
...
def confirm_stage(part_id, stage_id):
    ...
    if form.validate_on_submit():
        ...
        remaining = part.quantity_total - part.quantity_completed
        if quantity_done > remaining:
            flash(f'Ошибка: Нельзя выполнить {quantity_done} шт. '
                  f'Осталось всего {remaining} шт. в партии.', 'error')
...
def edit_note(note_id):
    ...
    if note.user_id != current_user.id and not current_user.is_admin():
        flash('У вас нет прав для редактирования этого примечания.', 'error')
    ...
    if new_text and new_text.strip():
        ...
        flash('Текст примечания не может быть пустым.', 'error')
